...
async function validateS3BucketNames(input: Resources) {
  ...
  const listOfS3BucketsFromEnvironment =
    // eslint-disable-next-line n/no-process-env
    process.env['S3_BUCKET_NAME_LENGTH_EXCEPTIONS'] ?? undefined;
  const S3_BUCKET_NAME_LENGTH_EXCEPTIONS =
    listOfS3BucketsFromEnvironment === undefined
      ? new Set()
      : new Set(listOfS3BucketsFromEnvironment.split(','));
...
export async function validateNameAndResourceLength(
  packageJSONWithResources: PackageJSON,
): Promise<void> {
  ...
  const SERVICE_NAME_LENGTH_EXCEPTION =
    // eslint-disable-next-line n/no-process-env
    process.env['SERVICE_NAME_LENGTH_EXCEPTION'] ?? undefined;
  const serviceName = packageJSONWithResources.service.name;

  if (
    SERVICE_NAME_LENGTH_EXCEPTION !== serviceName &&
    serviceName.length > MAXIMUM_SERVICE_NAME_LENGTH
  ) {
